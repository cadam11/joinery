...
| Claim                                                                                                                                                                                                   | Source                                                                                                                       |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| One keychain item: service `ca.adam11.joinery`, account `credentials-vault`, holding JSON                                                                                                               | `packages/main/src/services/keychain/credential-store.ts:20, 30-35, 78-90`                                                   |
...
| A packaged app ignores the variable, uses the app id, and logs a warning that names neither service                                                                                                     | `packages/main/src/services/keychain/service-name.ts:76-89`, `packages/main/src/services/keychain/credential-store.ts:48-60` |
| It is read once at startup and cached, and concurrent callers share one read                                                                                                                            | `packages/main/src/services/keychain/credential-store.ts:62-76, 169-184`                                                     |
| A save rewrites the whole vault                                                                                                                                                                         | `packages/main/src/services/keychain/credential-store.ts:128-133, 138-155`                                                   |
| Legacy per-credential items are migrated into the vault and deleted, only when none exists                                                                                                              | `packages/main/src/services/keychain/credential-store.ts:91-110`                                                             |
| A failed read marks the store unavailable, continues, and logs that exact sentence                                                                                                                      | `packages/main/src/services/keychain/credential-store.ts:113-122`                                                            |
| A failed write keeps the value in memory for the session and logs it                                                                                                                                    | `packages/main/src/services/keychain/credential-store.ts:146-163`                                                            |
| A failed delete logs its own line and still removes the entry from memory                                                                                                                               | `packages/main/src/services/keychain/credential-store.ts:206-212`                                                            |
| The degradation is announced once per session, and carries availability only                                                                                                                            | `packages/main/src/services/keychain/credential-store.ts:229-257`                                                            |
| Availability never returns within a session — there is no retry                                                                                                                                         | `packages/main/src/services/keychain/credential-store.ts:242-244` (the flag is only ever cleared)                            |
