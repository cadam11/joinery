...
| Query timeout (seconds)      | 30 — **disabled** | Not in use. See below.                                                                      |

**Query timeout ships disabled.** Nothing reads it: the effective timeout comes from the connection
profile's own request timeout, and the control says so rather than pretending. Making it live is
tracked as J-54.
