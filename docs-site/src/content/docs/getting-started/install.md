...
exists. A stamped bundle honours `JOINERY_KEYCHAIN_SERVICE`, so the smoke run points the
credential vault at a throwaway keychain service instead of the one holding your real passwords,
and it honours `JOINERY_TEST=1`, so the run stays headless. A release bundle refuses both. It is a
property of the artifact rather than of the environment on purpose: anyone who could set one
...
| A stamped bundle honours `JOINERY_KEYCHAIN_SERVICE` and `JOINERY_TEST`; a release bundle refuses both | `packages/main/src/services/keychain/service-name.ts:50-65, 96-107`; `packages/main/src/utils/runtime-mode.ts` (`RuntimeSignals.isTestBuild`, `isTestHatchOpen`); `packages/main/src/utils/test-build-capability.ts:53-58` |
