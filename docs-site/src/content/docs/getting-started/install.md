...
| A stamped bundle honours `JOINERY_KEYCHAIN_SERVICE` and `JOINERY_TEST`; a release bundle refuses both | `packages/main/src/services/keychain/service-name.ts:50-65, 97-108`; `packages/main/src/utils/runtime-mode.ts` (`RuntimeSignals.isTestBuild`, `isTestHatchOpen`); `packages/main/src/utils/test-build-capability.ts:53-58` |
