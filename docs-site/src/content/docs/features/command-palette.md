...
| ⌘K reaches the palette from inside the SQL editor, which Monaco used to prevent    | `packages/renderer/src/editor/sql-editor.tsx:452-493`, `editor/sql-editor.spec.tsx` — 'releases ⌘K, so the palette’s own shortcut reaches the window' |
