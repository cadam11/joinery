...
**There is no cancel button yet.** Closing the dialog does not stop the dump — it finishes in the
background, and the dialog says so. The button was withheld because cancelling did not cancel:
every cancel was routed to the SQL Server service, which does not hold the PostgreSQL or MySQL
operations, so it stopped the progress readout and left `pg_dump` running. That routing is fixed,
and a cancel now reaches the child process those two engines spawn. The button follows once the
dialogs are wired to it. SQL Server remains uncancellable: its backup runs inside the server, not
in a process Joinery owns.
...
### PostgreSQL and MySQL restores are checked against the server

Neither `mysql` nor `pg_restore` is a reliable witness to its own success. An empty or invalid dump,
a missing `CREATE` privilege, a target `pg_restore` was never told to create — all of them can end
with an exit code that looks like success and nothing applied. So when the tool exits, Joinery
...
so a restore that succeeded against such a server is reported as a success.
...
| The preview carries `INIT` and `STATS = 5`, and why                                    | `packages/renderer/src/features/backup/backup-model.ts:185-225`                                                                                                                             |
| The preview is checked against the statement the main process actually builds          | `tests/fixtures/tsql-preview/mssql-statements.sql` (generated), `packages/main/src/services/sql/mssql-preview-fixture.spec.ts`, `packages/renderer/src/features/tsql-preview-drift.spec.ts` |
...
| An indeterminate bar when the tool reports no percentage                               | `packages/renderer/src/features/backup/backup-model.ts:394-407`, `backup-dialog.tsx:720-762`                                                                                                |
