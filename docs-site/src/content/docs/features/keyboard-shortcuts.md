...
**Control-M** is the way out — the same keystroke on macOS and on Windows. Press it and Tab moves
focus to the next element instead of indenting; press it again and Tab indents again. The editor
announces which mode it is in each time, so a screen reader says what changed.

It is bound inside the editor rather than in the command table, so it appears neither in the ⇧⌘/
sheet nor in the [keyboard shortcuts reference](../../reference/keyboard-shortcuts/), both of which
...
| Claim                                                                              | Source                                                                                                                               |
| ---------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| Control-M toggles tab-focus mode, bound in the editor and not in the command table | `packages/renderer/src/editor/sql-editor.tsx:360-397`                                                                                |
